...
- Available tools on server: `curl`, `nats`, `grep`, `awk`, `sed`
...
ssh root@$HUB "curl -s localhost:8096/health"
```
...
```bash
ssh root@$HUB "nats sub 'land.status.$TEST_ORG' --server nats://127.0.0.1:4222" &
ssh root@$HUB "nats sub 'cloud.>' --server nats://127.0.0.1:4222" &
```

Note: if NATS requires auth, use a default account credential from mycelium.
...
curl -s -X POST http://$HUB:8096/api/v1/lands \
  ...
  -d "{\"org_slug\":\"$TEST_ORG\",\"org_name\":\"$TEST_ORG_NAME\"}"
```
...
- [ ] Response contains `"status":"provisioning"`
- [ ] Response contains `"server_ip":`
...
SERVER_IP=$(curl -s -H "Authorization: Bearer $ADMIN_TOKEN" \
  http://$HUB:8096/api/v1/lands/$TEST_ORG | grep -oP '"server_ip":"\K[^"]+')
...
```bash
ssh root@$HUB "curl -s http://127.0.0.1:8222/leafz | grep -i e2etest"
```
